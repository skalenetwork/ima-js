...
describe("ERC20 S2S transfer flow", () => {
    ...
    before(async () => {
        ...
        mainnet = test_utils.initMainnet();
        ...
        wETHToken = test_utils.wETHToken(sChain1.web3);
    ...
    it.only("Test ETH transfer with wrap/unwrap", async () => {
        // transfer ETH from Mainnet to chain1

        let transferOpts: TxOpts = {
        ...
        let sChainBalanceBefore = await sChain1.ethBalance(address);

        await mainnet.eth.deposit(
            test_utils.CHAIN_NAME_SCHAIN,
            transferOpts
        );
        await sChain1.waitETHBalanceChange(address, sChainBalanceBefore);

        let mainnetBalanceAfterDeposit = await mainnet.ethBalance(address);
        let sChainBalanceAfterDeposit = await sChain1.ethBalance(address);
